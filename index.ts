const x1 = document.getElementById("x1") as HTMLInputElement
const y1 = document.getElementById("y1") as HTMLInputElement
const x2 = document.getElementById("x2") as HTMLInputElement
const y2 = document.getElementById("y2") as HTMLInputElement
...
function new_rec(): void{
    if (!(color.value === "" || x1.value === "" || x2.value === "" || y1.value === "" || y2.value === "")){
        const rec: Rectangle = {color: color.value, 
...
function add_rec(rectangle: Rectangle): void {
    to_default()
    if (rectangle.x2 - rectangle.x1 > 0 && rectangle.y2 - rectangle.y1 > 0){
        ...
        pict?.appendChild(new_rec)
    }
}


function clicked(x: number, y: number): void{
    click_x = x
    click_y = y
}


function unclicked(x: number, y: number): void{
    if (click_x != -1 && click_y != -1 && x != click_x && y != click_y){
        let col = "black"
        if (!(color.value === "")){
            col = color.value
        }
        const rec: Rectangle = {color: col, 
...
function del(x: number, y: number): void{
...
pict?.addEventListener("mousedown", (event) => {clicked(event.offsetX, event.offsetY)});
pict?.addEventListener("mouseup", (event) => {unclicked(event.offsetX, event.offsetY)});
pict?.addEventListener("click", (event) => {del(event.offsetX, event.offsetY)});